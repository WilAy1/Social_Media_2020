...
if(!file_exists("../../../students_connect_hidden/users_profile_upload/".$row['user'].'/'.$row['user'].".png")){
  ...
  echo "<div class='dmyimg'><div class='myimg'  style='background-image: url(\"$mmg\"); background-size: 100%; 
  repeat: no-repeat; height:50px; width:50px'></div></div>";
}
else{
...
echo 
  '<div class="shwname">
  <a href="/students_connect/profile.php" style="text-align: left;" id="shwname">
  <div class="web_nm">
  <div class="web_nn">'.$row['firstname']. ' ' .$row['surname'].'</div>
  <div class="web_usr"><i class="fas fa-at"></i>'.$row['user'].'</div>
  </div></a></div></div>

  <hr style="width:100%; border-bottom:1px;">
  <div class="pt2" style="width: 29%;">
  <div class="listls">
  ...
  <div class='smpbu txgd'><i class='fas fa-search icf'></i>
  ...
  echo "<div class='mnt_ev tgd'>
  <div class='smpbu txgd'><i class='fas fa-ad icf'></i>
...
$sltm = queryMysql("SELECT * FROM eduposts WHERE (user IN ($win) or sharedby IN ($win)$nstr $surelyoccured) 
    ...
    AND timeofupdate BETWEEN $lthidays AND $ptime
    ORDER BY timeofupdate DESC LIMIT 10");
      if($sltm->num_rows == 0){
...
while(($medu = mysqli_fetch_array($sltm)) && $e < count($medu)){
  ...
  for($x = 0; $x < count($suggestedspace); $x++){
    if($suggestedspace[$x] == $e){
      ...
      $fsg = queryMysql("SELECT * FROM members WHERE $nts AND $occr ORDER BY RAND() LIMIT 5");
      ...
      if(file_exists($gfsg['user'].".png")){ 
        ...
        echo "
           ...
           echo "</a><div class='o_m_d s_trbt1s'>
    ...
    <div id="myDropdown$sid" class="std_yx">
            ...
            <div class="namp">';
                ...
                if(file_exists($fme['user'].".png")){ 
                  $img =  '/students_connect_hidden/users_profile_upload/'.$row['user'].'/'.$fme['user'].'.png';  
                }
                  else {
                    chdir($td);
          ...
          elseif(($ctime - $ttime) > 86400 && ($ctime - $ttime) < 604800){
            ...
            $ftime = date("M d h:i a", $ttime);
                ...
                if(file_exists($mee['user'].".png")){ 
                  $fnimg =  '/students_connect_hidden/users_profile_upload/'.$row['user'].'/'.$mee['user'].'.png';  
                }
                  else {
                    chdir($td);
          ...
          $mbse = mysqli_fetch_array(queryMysql("SELECT * FROM members WHERE user='".$medu['user']."'"));
              ...
              if($data == 1){
                ...
                $geteducomment = mysqli_fetch_array($educomment);
                  ...
                  echo '<div class="addcom" id="addcom"><div class="wcb"><div class="cmttxt">
...
if(file_exists("../../students_connect_hidden/users_profile_upload/".$post['user'].".png")){ 
  ...
  if($data == 1){
    ...
    <div class='nofviews'>".$nofv."</div></div>";
      ...
      </div><button onclick="sendShare(\''.$dsm['user'].'\', \''.$post['pstcont'].'\', \''.$post['id'].'\')">Send</buton>
      <div class="shasp">
...
echo <<<GODMAKEMYWORKPROSPER
    <input type='hidden' value='12' id='pnum'>
    <div id='more'></div>
    <div class='fp_pplw'>
    <div class='f_eppee'>Fetching Posts. Please Wait</div></div>
    <div id='br_kk_ea'>
    </div>
    </div>
    <div class='mspst'>
...
function lmore(){
    ...
    function(data){
      ...
      for(var i =0; i < document.scripts.length; i++){
        ...
        if(document.scripts[i].src.includes('student_connect.js')){
          var oxe = document.createElement('script');
          oxe.src = '/students_connect/jsf/student_connect.js';
          document.scripts[i].replaceWith(oxe);
        }
      }
  })
...
var ax = document.getElementsByTagName('A');
  ...
  for(var i = 0; i < ax.length; i++){
      ax[i].onclick = function(e){
          e.preventDefault();
          var ee = this.href;
          var t = document.getElementsByClassName('hom_tw')[0];

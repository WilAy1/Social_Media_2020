<?php
    define("look", "/Users/wilay/students_connect/");
    require_once look."connect.php";
    require_once look.'fmnu.php';    
    if(isset($_GET['pid'])){
            if(isset($_SERVER['HTTP_REFERER'])){
                echo '<div class="lg_b">
                <input type="hidden" value="'.$_SERVER['HTTP_REFERER'].'"
                <i class="fas fa-arrow-back"></i></div>';
            }
            else {
                echo '<div class="lg_b">Forum Home</div>';
            }
            $mbi = mysqli_fetch_array(queryMysql("SELECT * FROM members WHERE user='$user'"));
            $user = $mbi['user'];
            $id = sanitizeString($_GET['pid']);
            $fid = sanitizeString($_GET['fid']);
            $xed = mysqli_fetch_array(queryMysql("SELECT * FROM forums WHERE id='$fid'"));
            $egp = queryMysql("SELECT * FROM forumposts WHERE id='$id' AND forumid='$fid'");
            $me = queryMysql("SELECT * FROM forumscomment WHERE pid='$id' AND fid='$fid'");
            $gp = mysqli_fetch_array($egp);

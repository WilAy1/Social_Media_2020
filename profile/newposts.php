<?php
    ...
    if(isset($_GET['user']) && isset($_GET['lt'])){
        $user = sanitizeString($_GET['user']);
        if(!empty($_GET['lt'])){
        $ltime = sanitizeString($_GET['lt']);
        }
        else { 
          $ltime = '0';
        }
        $ctime = time();
        $checking = sanitizeString($_GET['checking']);
        $upc = mysqli_fetch_array(queryMysql("SELECT * FROM members WHERE user='$user'"));
        $row = $mbs = mysqli_fetch_array(queryMysql("SELECT * FROM members WHERE user='$checking'"));
        $edu = queryMysql("SELECT * FROM eduposts WHERE  (user='$user' AND pstst=0 AND sharedby='$user' 
        AND timeofupdate > $ltime AND timeofupdate < $ctime) OR
        (user != '$user' AND sharedby = '$user' AND pstst=0 
        AND timeofupdate > $ltime AND timeofupdate < $ctime) 
         UNION ALL
         SELECT * FROM socposts WHERE (user='$user' AND pstst=1 AND sharedby='$user' 
         AND timeofupdate > $ltime AND timeofupdate < $ctime) OR
        ...
        if($row['status'] == '1' || $row['status']== '2'){
            if(!empty($edu)){
                while($medu = mysqli_fetch_assoc($edu)) { 
                     ...
                     $polc = queryMysql("SELECT * FROM polls WHERE pid='$tpeid' AND timetoend > '$etime' AND pstst='0'");
                      ...
                      if($data == 1){
                        ...
                        chdir($td);
                          ...
                          <input type='hidden' value='0'>
                          <input type='hidden' value='".$medu['id']."'>
                          <input type='hidden' value='".$mbs['user']."'>
                          <i class='fas fa-comment'></i></div>";
                          $xzt = "<div class='tb_y repop'>Report Post</div>";
                          $xyt = "<div class='tb_y blusr'>Block User</div>";
                          if($medu['sharedby'] == $mbs['user']){
                  ...
                  echo nl2br($pstcut).'</div>';
                     ...
                     <div class="ploxx">
                          ...
                          $scusr = $row['user'];
                          $mffs = queryMysql("SELECT * FROM messagesbase WHERE fone='$scusr' OR ftwo='$scusr' order by lasttimeofmessage desc limit 5");
                          if($mffs->num_rows==0){
                          ...
                          $mfss = queryMysql("SELECT * FROM followstatus WHERE user='$scusr' AND type='following'");
                          if($mfss->num_rows==0){
                            echo 'No recently messaged user';
                          }
                          else {
                ...
                $lvd = queryMysql("SELECT * FROM loves WHERE user='$xerw'
                 AND loved='liked' AND id='".$medu['id']."'");
                 $dlkd = queryMysql("SELECT * FROM loves WHERE user='$xerw'
        ...
        elseif($row['status'] == '3'){
                $soc = queryMysql("SELECT * FROM socposts WHERE (user='$user' AND pstst=1 AND sharedby='$user' 
                AND timeofupdate > $ltime AND timeofupdate < $ctime) OR
               (user != '$user' AND sharedby = '$user' AND pstst=1 
               AND timeofupdate > $ltime AND timeofupdate < $ctime)
               ORDER BY timeofupdate DESC");
